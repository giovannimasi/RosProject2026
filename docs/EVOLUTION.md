...
| At every goal the robot crawled senselessly near the just-reached goal instead of heading to the next one, with the highlighted path frozen at an old start point for many cycles | The **memorized action** (re-offering the previous trajectory) was identical to the **hysteresis reference**, so it always scored zero hysteresis penalty and beat every fresh RRT path — locking the selection onto a stale, old-start trajectory even as the robot moved. Fresh paths were computed correctly but never chosen | **Removed the memorized action.** Continuity now comes from hysteresis alone (comparing *fresh* candidates to the previous choice) plus the path-following controller; the selected path is always fresh from the current pose. Dead memorized fields cleaned up |
